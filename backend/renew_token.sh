update_env() {
  ...
  if grep -q "^$key=" ~/project/src/backend/.env; then
    sed -i "s/^$key=.*/$key=$value/" ~/project/src/backend/.env
  
  else
    echo "$key=$value" >> ~/project/src/backend/.env
  fi
}
...
source ~/project/src/backend/.env
